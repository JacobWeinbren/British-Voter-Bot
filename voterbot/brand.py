...
def intro_poster_alt(respondents: str, cadence: str) -> str:
    return (
        "Introduction poster for British Voter Bot. Headline: Real British voters, one at a time. "
        "Every card on this account is one real, anonymous respondent to the British Election Study, "
        "Britain's largest survey of voters; each card says who they are, what they think and how they'd vote, "
        "in their own answers to questions written by the BES. The views are the respondent's own, not ours. "
        "How to read a card: the dot on the map is where they live, marked at their "
        "constituency; the speech bubbles are their views - favourite and least favourite leader, the issues they "
        "care about, how they see their nation - each one the respondent's own answer to a BES question, not the "
        "opinion of this account, its authors or the BES; two scales show where they sit on the BES 0 to 10 economic left-right and "
